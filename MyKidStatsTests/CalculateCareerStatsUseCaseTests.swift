...
final class CalculateCareerStatsUseCaseTests: XCTestCase {
    ...
    func testCalculateCareerStats() async throws {
        ...
        let game1 = TestDataHelper.createTestGame(team: team, focusChild: child, context: context)
        game1.isComplete = true
        let event1 = StatEvent(context: context)
        event1.id = UUID(); event1.playerId = player.id; event1.gameId = game1.id
        event1.timestamp = Date(); event1.statType = StatType.twoPointMade.rawValue; event1.value = 2; event1.isSoftDeleted = false

        let game2 = TestDataHelper.createTestGame(team: team, focusChild: child, context: context)
        game2.isComplete = true
        let event2 = StatEvent(context: context)
        event2.id = UUID(); event2.playerId = player.id; event2.gameId = game2.id
        event2.timestamp = Date(); event2.statType = StatType.threePointMade.rawValue; event2.value = 3; event2.isSoftDeleted = false

        try context.save()

        // Act
        let useCase = CalculateCareerStatsUseCase(context: context)
        let statsResult = try await useCase.execute(for: child.id!)
        let stats = try XCTUnwrap(statsResult)
